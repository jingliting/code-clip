...
def threading_data(data=None, fn=None, thread_count=None, **kwargs):
    ...
    if thread_count is None:  # 有多少数据开多少线程
        results = [None] * len(data)  # preallocate result list
        threads = []
        for i, d in enumerate(data):
            t = threading_data.Thread(name='threading_and_return', target=apply_fn, args=(results, i, d, kwargs))
            t.start()
            threads.append(t)
    else:
        divs = np.linspace(0, len(data), thread_count + 1)  # 实现间隔采样
        divs = np.round(divs).astype(int)  # 舍入
        results = [None] * thread_count
        threads = []
        for i in range(thread_count):
            t = threading_data.Thread(
    ...
    if thread_count is None: # TODO
        try:
            return np.asarray(results)
        except Exception:  # if dim don't match
            return results
    else:
        return np.concatenate(results)  # 数组拼接
